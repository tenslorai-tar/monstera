...
/** Where TypeScript that vitest may collect lives. */
...
function sourceFilesIn(dir) {
  ...
  try {
    ...
    // the caller's scope check rather than swallowed here: "there is no
    // packages/" and "packages/ has no TypeScript" must not reach the verdict
    // as the same thing.
    return found;
  }
...
export function scan({ root = repoRoot() } = {}) {
  ...
  for (const dir of ROOTS) {
    for (const path of sourceFilesIn(join(root, dir))) {
...
export function report(result) {
  if (result.violations.length === 0) {
    return (
      ...
      `  ${String(result.filesScanned)} TypeScript file(s), ` +
...
export function run({ root = repoRoot() } = {}) {
  ...
  if (result.filesScanned === 0) {
    process.stdout.write(
      `  FAIL  no TypeScript file was found under ${ROOTS.join(', ')}.\n` +
        `        An empty file list is a broken walk, not a clean tree, so this refuses to\n` +
        `        report a result.\n`,
    );
    return 1;
  }
