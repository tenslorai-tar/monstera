  ...
  const ACCOUNTED_COMPUTED = new Map([
    ...
    [
      'scripts/research/lowboxSpike.mjs',
      {
        sites: 6,
        reason:
          'imports six built modules through file:// URLs, and the count has moved twice: ' +
          '2 to 4 when pipe creation followed process creation onto the shipped surface, and ' +
          '4 to 6 on 2026-08-26 when directory creation followed them (ADR-0023 Decision 12). ' +
          'All six are the same argument: reading the build IS the measurement, and a ' +
          'hand-rolled equivalent beside it is the B3a defect these moves remove. ' +
          ...
          'apps/desktop/dist/win32DirectorySurface.js and dist/sessionDirectories.js create ' +
          'the handed pair, so the four verb-split rows measure a DACL shipped code wrote ' +
          'rather than one this instrument assembled with icacls — the transfer this ' +
          'repository already refused once for invariant 25(b). ' +
          ...
          'satisfied rather than evaded — and none of the five built modules imports it ' +
    ...
    [
      'scripts/research/readerWorker.mjs',
      {
        sites: 4,
        reason:
          'imports apps/desktop/dist/win32PipeSurface.js, dist/enginePipeFactory.js, ' +
          'dist/readerHostSurface.js and dist/engineReaderChannel.js through file:// URLs, so ' +
          'that it drives the SHIPPED composition — the channel creates the stop event and ' +
          'starts the reader — rather than a hand-rolled Worker beside it. The reader itself is ' +
          'started by PATH, not imported, so it is not a site. All four need Windows backslash ' +
          'conversion at run time, and none of the built modules imports the electron package.',
      },
    ],
    [
