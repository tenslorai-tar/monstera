...
/** Filler, kept separate for {@link MOVE_SPEC}'s reason. */
const BATES_SPEC = `  batesNumberPages: {
...
/**
 * The newest kind, kept separate for {@link MOVE_SPEC}'s reason — the
 * missing-a-kind case omits it.
 *
 * The first spec whose command is neither a page-tree operation nor a drawing
 * onto the content stream: it writes an object into `/Annots`. Nothing about
 * the table's shape distinguishes those, which is the point — a third kind of
 * effect registers the same way the first two did.
 */
const ANNOTATION_SPEC = `  addAnnotation: {
    kind: 'addAnnotation',
    writer: 'mupdf',
...
/**
 * The newest kind, and the one the `missing a command kind` case omits.
 *
 * `invertible: true` with `undo: 'inverse'`, unlike every annotation-adjacent
 * spec beside it: a fill names one widget, so its prior is one value. And like
 * {@link PLACE_IMAGE_SPEC} it spells no `targets`, because `CommandSpec` does
 * not carry that axis — the bus branches on the declaration before this table
 * is reached.
 */
const FILL_SPEC = `  fillFormField: {
    kind: 'fillFormField',
    writer: 'mupdf',
...
export const handlers: ContractHandlers = {
  ...
  'document.print': () => Promise.resolve(ok({ kind: 'cancelled' as const })),
  'document.email': () => Promise.resolve(ok({ kind: 'unavailable' as const })),
  'document.exportPdfa': () => Promise.resolve(ok({ kind: 'cancelled' as const })),
  ...
  'settings.export': () => Promise.resolve(ok({ kind: 'cancelled' })),
  ...
  'cloud.status': () => Promise.resolve(ok({ providers: [] })),
  'cloud.signIn': () => Promise.resolve(ok({ kind: 'done' })),
  'cloud.signOut': () => Promise.resolve(ok({ state: 'signed-out' })),
  'cloud.list': () => Promise.resolve(ok({ kind: 'listed', files: [] })),
  'cloud.open': () => Promise.resolve(ok({ kind: 'refused', reason: 'not-configured' })),
  'cloud.saveBack': () => Promise.resolve(ok({ kind: 'not-from-cloud' })),
  'cloud.uploadCopy': () => Promise.resolve(ok({ kind: 'done' })),
