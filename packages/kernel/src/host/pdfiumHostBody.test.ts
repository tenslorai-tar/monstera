...
function start(files: Files, applied: ByteImage = new Uint8Array([9, 9, 9])) {
  ...
  const handlers = createPdfiumHandlers({
    ...
    parses: async (image) => {
      calls.push(`parse:${[...image].join(',')}`);
      await Promise.resolve();
      // A DOCUMENT THIS ENGINE CANNOT READ is a single byte 0 here, so the
      // open-failed case exercises a refusal from the parse rather than from
      // the file read — the two are different failures at the same code, and
      // only a case that separates them says the parse happens at all.
...
describe('the PDFium host body', () => {
  it('opens an area, parses the image ONCE, and keeps no session', async () => {
    stream = stubStream();
    const files = emptyFiles();
    const { session, calls } = await openArea(files);

    expect(session.length).toBeGreaterThan(0);
    // THE PARSE HAPPENED (ADR-0048 Decision 3). Without this the open is
    // indistinguishable from one that validates nothing, and `open-failed`
    // would mean something different from this host than from MuPDF's.
    expect(calls).toStrictEqual(['parse:1,2,3']);
  });

  it('answers open-failed when the engine cannot read the image, not when the FILE is missing', async () => {
    stream = stubStream();
    const files = emptyFiles();
    // A file that exists and does not parse. The pair matters: a case using a
    // MISSING file would pass against a host whose open never parses, since
    // both produce `open-failed`. This one is refused by the parse.
    files.read.set(`${AREA.snapshotDirectory}|bad`, new Uint8Array([0]));
    start(files);
    stream.feed(
      request('o1', 'engine/open', {
        snapshotDirectory: AREA.snapshotDirectory,
    ...
    expect(answerIn(stream.sent[0])).toMatchObject({
      body: { ok: false, error: { code: 'open-failed' } },
    });
  });
  ...
  it('applies from the named file and writes the result into the granted directory', async () => {
    ...
    expect(calls).toStrictEqual(['parse:1,2,3', 'apply:4,5']);
  ...
  it('captures prior state from the named file and writes nothing', async () => {
    ...
    const files = emptyFiles();
    const { session, calls } = await openArea(files);
    files.read.set(`${AREA.snapshotDirectory}|${IN}`, new Uint8Array([6]));
    ...
    expect(calls).toStrictEqual(['parse:1,2,3', 'capture:6']);
  });
  ...
  it('answers the page’s text objects, from the named file', async () => {
    ...
    expect(calls).toStrictEqual(['parse:1,2,3', 'text-objects:3:7']);
  });
