...
// `hostPipeDacl`'s own cases live in `hostDacl.test.ts` with the function
// (finding EEEE-1). It is still called below, because what THIS file proves is
// that the factory passes the resolver's output to `describe` unaltered — which
// needs the resolver, and would be a second opinion if it restated the literal.
