...
| CommandBus — the log as a cursor, inverses recording prior state, invertibility and reproducibility declared per command. **Build ADR-0009 §7's per-document serial lane before or with the version counter, never after** — two commands both reading version N and both writing N+1 is the race the lane closes, and adding the lane underneath version logic already built is architecture retrofitted under a feature. | — |
