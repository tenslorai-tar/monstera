...
| Contract: the four surfaces **implemented** — main handlers, preload bridge, renderer client, browser shim. **Browser shim done** (`packages/testing`): a full `ContractClient`, derived by `createClient` from the registry so an unimplemented channel is a compile error, running the real `wrapHandlers` boundary rather than canned envelopes, and structured-cloning both directions so it rejects what the wire rejects. **Main handlers partial** — `executeCommandHandler` is annotated `ContractHandlers['document.execute']`, but no assembled `ContractHandlers` map is registered with `ipcMain` and `app.info` has no main-side handler. **Preload bridge and renderer client absent**; `packages/ui/src/index.ts` is still a bare `export {}`. (This row previously said all three packages were bare, which stopped being true when the composition point landed.) | — |
