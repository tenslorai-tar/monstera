...
| **Engine host containment asserted against a RUNNING process** (invariant 25). Not against the options passed to `utilityProcess.fork` — a flag that did not take effect and one that did are indistinguishable until it matters, which is the whole lesson of the compiler-mitigations check reading the PE image rather than the build flags. Three assertions, each on a live host: (a) its **integrity level** is the lowest workable one, read back from the process token **by main, against the child's token — not by the host against its own**; (b) its **job object** bounds memory and forbids process creation, read back from the job **and from behaviour, beside a control that does not assign one**; (c) a **network connection attempt from inside the host fails** — the only form of "no network" that is evidence rather than configuration. **Measured 2026-08-21** (`scripts/research/`), and two of the three now have a mechanism and a fixture rather than a description. (a): a host CAN lower its own integrity — `SetTokenInformation` to Low returns true — but afterwards **cannot read its own token at all**, `OpenProcessToken` failing with ACCESS_DENIED because the process object's descriptor was created at Medium. That is a *could-not-look*, consistent with the lowering having worked and no evidence that it did, so the read-back has to come from outside; the wording above said only "the process token" and the obvious reading of it is impossible. (b): obtained and **behaviourally verified** — with `ActiveProcessLimit = 1` and `ProcessMemoryLimit = 512 MB` assigned from main, the host cannot spawn and cannot commit 768 MB, where the same host with no job spawns `v43.4.1` and allocates 768 MB. Chromium **already** places a utility process in a job, so ours nests, and it takes effect. (c): the fixture is settled — a **loopback** connection the host makes to itself, which succeeds unconstrained, so a refusal cannot be a runner with no network (HH-2). Its mechanism is not: `session` governs Electron's `net` module and says nothing about `require('node:net')`, and integrity is a write control — a Low host still connected. Likewise the filesystem property: Node's permission model is **withdrawn**, since `--permission` is accepted by `fork`, arrives in the host's own `process.execArgv`, and leaves `process.permission` absent with `fs` reachable — set, readable back as set, and inert. The declaration in `docs/security/engine-advisories.json` (`engine-host-containment`) turns the build red the day shipped code names `utilityProcess`, which is what forces this row rather than leaving it to whoever writes the host. That trigger catches "a host was written"; this row is "and it was contained". Its symbols were the project's only UNVERIFIABLE ones while Electron was not a dependency, and **all three predicted consequences landed together on 2026-08-20**: the `in: null` expired, a witness became possible, and the list stopped being hand-picked. They are now **derived** from `electron.d.ts` by the TypeScript compiler (`scripts/security/electronSurface.mjs`), not witnessed — a derivation is stronger, and a witness here would digest a 1.1 MB file and report that it *changed* rather than that the symbol is still declared in it (finding T-1). **The derivation immediately proved the hand-picked list short**: it reports the spawn surface by TYPE — every declaration typed as the utility-process factory — which is `utilityProcess` *and* `UtilityProcess`, and the class was missing. A PARSE and not a search, because the file is 56% comments and on 43.4.1 `utilityProcess` occurs once in prose: a text match would witness a symbol that had been **removed**, which is the reassuring answer. What it witnesses is **declarations only** — every workflow installs with `--ignore-scripts`, which fetches the 1,125,720-byte `.d.ts` and no binary at all, so nothing here says an Electron process can start. Where `node_modules` is absent (the Guards job) the symbols report as unverifiable; `--require-derivation`, in the one job that installs, makes them mandatory. | — |
