...
| **Gate:** performance budget assertion — 200 MB fixture, **per-process** peak RSS against invariant 17: main ≤ 1.5× **and** ≤ 1.5 GB, MuPDF host ≤ 6× **and** ≤ 3 GB as a containment limit whose breach means kill-and-restart. IPC bounded per L11. **The renderer half cannot be asserted yet** — invariant 17 makes it provisional and two-term (a file-size-proportional term plus an absolute bitmap-cache cap), both unmeasurable until a renderer exists; the single ≤ 2.5× figure this row used to carry is recorded in ADR-0007 as the mistake it was, having no derivation. | — |
