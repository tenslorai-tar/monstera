...
**Measured 2026-08-21** (`scripts/research/`). **(a) and (b) have a mechanism, a fixture and a differential. (c) and (d) have none** — (d) had a candidate and it fell to QQ-1. (a): a host can lower its own integrity, and afterwards cannot read its own token at all — `OpenProcessToken` fails ACCESS_DENIED, because the process object's descriptor was created at Medium. That is a *could-not-look*, so the reading comes from main, which sees the child at **0x2000 before and 0x1000 after** — two readings differing across the one action, which is the only shape that separates a working reader from one that always says Low. (b): obtained and **behaviourally verified** — under `ActiveProcessLimit = 1` and a process memory limit, the host cannot spawn and cannot commit past it, where the same host with no job spawns `v43.4.1` and allocates freely. Chromium **already** places a utility process in a job, so ours nests, and it takes effect. (d): **no mechanism.** Node's permission model was the candidate and it does not survive the adversary this invariant names. Measured in one process under one policy, with the model active and `--allow-addons` granted: a JavaScript read outside the allow-list is refused `ERR_ACCESS_DENIED`, and a **native** read of the same file through `CreateFileW` returns 4,096 bytes beginning `MZ` — both beside inside-the-allow-list reads that succeed, so neither result can be a broken call. **The dial has no setting that works:** without `--allow-addons` the host cannot load koffi at all and so cannot reach MuPDF; with it, native code is outside the model by design. One end has no engine, the other has no containment. (c): the **fixture** is settled — a loopback connection the host makes to itself, which succeeds unconstrained, so a refusal cannot be a runner with no network (HH-2) — and nothing yet delivers it: `session` governs Electron's `net` module and not `require('node:net')`; integrity is a write control and a verified-Low host still connected; the permission model does not cover network and `canListen` stays true with it enforcing.
