...
- **A grouping of our own is permitted where no engine answers, and only where a
  ...
  answers?** A second caller is the moment it has become an extraction path, and
  ADR-0049 stops covering it. Its extent leaves the kernel to place that editor
  and decides nothing in the renderer.
