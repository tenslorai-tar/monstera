...
- **A third host contains pure JavaScript, and its containment is established the
  same way** ([ADR-0060](DECISIONS/0060-an-imported-source-is-parsed-in-a-contained-host-that-holds-no-document.md),
  2026-09-13). The compose host parses a file picked for import with `markdown-it`
  and composes with `@cantoo/pdf-lib`, and binds no native library. Invariant 25's
  four properties belong to the token and the job object, not to what the process
  runs, so the mechanism is unchanged. *Both hosts* in the bullet above means the
  two writer hosts, which is all it meant when it was written. The compose host is
  not a writer: it takes the probe and a granted area's `open` and `close`, and is
  asked none of `apply`, `capture` or `invert`, which carry a writer's command
  union.
...
`exportAnnotations` the same day with D8's annotation import/export row
([ADR-0077](DECISIONS/0077-annotations-are-exchanged-as-dictionary-entries-in-user-space.md)),
and `accessibility-check` with D8's accessibility check
...
| A page from another open document placed onto a page as an optional-content layer (added 2026-09-14) | **MuPDF, through the object-tree writer `layers.ts` already is** — a Form XObject built from the source page with its inheritables pushed down and one graft map for the command, carrying `/OC`, drawn by an appended `q /Name Do Q`, its group appended to `/OCProperties/OCGs` and `/D/Order`. *Drawing onto pages* is `@cantoo/pdf-lib`'s, and taking it there would give `/OCProperties` a second writer ([ADR-0064](DECISIONS/0064-a-page-imported-as-a-layer-is-mupdfs-because-the-layer-is.md)). Measured 2026-09-14 on MuPDF 1.28.0 and read back by PDF.js 6.2.108: the layer is listed, draws the source's content at its place when on, and PDF.js hides it when `/D /OFF` names the group | MuPDF |
...
| Metadata, outline/bookmarks, encryption, permissions, redaction, optimize | **MuPDF**. **Optimize is MuPDF's own image rewriter, `pdf_rewrite_images`, reached NATIVELY through `monstera_mupdf.dll` and koffi in the compose host, writing a copy** (amended 2026-09-19, [ADR-0087](DECISIONS/0087-optimize-is-mupdfs-native-image-rewriter-in-the-compose-host.md)): the rewriter is not bound in the WASM build, the compose host holds no document so a native fault poisons nothing, and the open document is never changed — the sizes are compared before a destination is picked, and a result that is not smaller is not written. Measured over the corpus: −10.6%, −11.6% and −25.6% at the three settings, every page count held and 4 of 4 tagged documents keeping their structure tree, where Ghostscript dropped it from all four | MuPDF |
| Print & export rasterisation | **MuPDF**. A print is that raster of each page at the DPI the person chose, drawn by GDI (`StretchDIBits`) into the device context the operating system's print dialog (`PrintDlgExW`) answers, fitted into the printable area, from `main` — never the DOM (amended 2026-09-17, [ADR-0074](DECISIONS/0074-printing-is-mupdfs-raster-through-the-system-print-dialog-and-gdi.md)). Measured through *Microsoft Print to PDF*: each printed page differs from the raster sent for it by a mean 0.14 of 255, and from the other page's by 17.5 | — |
| **PDF → Word, PowerPoint and Excel** (`.docx`, `.pptx`, `.xlsx`; added 2026-09-17 ahead of D10's Office rows) | **This build's own Office Open XML parts, zipped by `fflate`, composed in `main` a page at a time and streamed to the destination** from the substrate's MuPDF reads, so what `main` holds is ADR-0035's page bound, as the plain text export's is (corrected 2026-09-17 from *composed in the engine host*) ([ADR-0072](DECISIONS/0072-office-open-xml-exports-are-written-by-this-build-over-fflate.md)). MuPDF writes none of these formats. `exceljs`, `docx` and `pptxgenjs` each reach packages that ship no licence text (all three through `jszip` → `readable-stream` → `isarray@1.0.0`), which `generateNotice.mjs` refuses, so Part A's *"use exceljs"* cannot be met alongside its notice rule; `xlsx` stays refused. Each format is written to the subset its export needs, every text value escaped by one function, and proven by opening the result in the application that owns the format | — |
| Reading and generating barcodes (added 2026-09-17 for D10's barcode row) | — (read-only; a generated barcode is written by `placeImage`, MuPDF's) | **zxing-cpp, through `zxing-wasm` 3.1.4**, loaded from its own `.wasm` bytes ([ADR-0076](DECISIONS/0076-barcodes-are-zxing-cpps-read-in-the-engine-host-written-for-the-place-image-command.md)). MuPDF's own barcode support is in its native build and not in the WASM build the kernel loads. **Reading runs in the engine host**, on MuPDF's raster of the page, because a document's pixels are input to a C++ decoder; `main` receives formats, text and boxes, bounded. **Generating runs in `main`, loaded on first use**: its input is a person's text, and the PNG it makes is the place-image command's bytes. Measured: QR, Code 128 and Data Matrix written, placed on a page, rasterised and read back exactly; a blank page reads none. The package ships only its MIT text, so zxing-cpp's Apache-2.0 and zint's BSD-3-Clause texts are rendered beside it |
| Finding a table on a page (added 2026-09-17 for D10's Excel rows) | — (read-only) | **MuPDF's `FZ_STEXT_TABLE_HUNT`, asked as MuPDF's own CSV writer asks it**: the substrate's `table` read, the shared options plus `vectors`, `accurate-bboxes` and `table-hunt`, carried by the page-text channel's closed `read` field and parsed by the one walk ([ADR-0073](DECISIONS/0073-a-table-is-the-engines-table-read-asked-as-its-own-table-writer-asks.md)). Without `vectors` a ruling line proposes no table and every generated grid came back two columns wide; with it, four of four ruled grids are exact. **An unruled table is still split at its gutters** (0 of 2), and a grid of our own over lines or word boxes is ADR-0034's refused clusterer. The automatic engine reads the page's text as it is. **A scanned table is read by a service that answers tables** ([ADR-0086](DECISIONS/0086-a-scanned-table-is-read-by-a-service-that-answers-tables.md), amended 2026-09-19): **Azure's Layout model** (`prebuilt-layout`) and **Claude asked for the table as structured output**, each reporting cells with their row, column and spans, in `main` and through the one client each service already has. **Tesseract is not a table engine**: it answers words and lines, so its only table would be the grid over word boxes ADR-0034 refuses |
| **Page image → WebP encoding** (added 2026-09-16 ahead of D10's WebP half) | **`@jsquash/webp`'s libwebp encoder, WASM, executing inside the engine host over the pixmap the row above produced** ([ADR-0070](DECISIONS/0070-a-page-image-is-encoded-as-webp-by-libwebps-wasm-in-the-engine-host.md)). MuPDF 1.28.0 and Electron's `nativeImage` encode no WebP, so the concern had no writer. The rasteriser is not duplicated; this row only turns MuPDF's pixels into bytes. It runs in the host for OCR recognition's reason — its input is a bitmap we produced, so containment is unchanged and the page's RGBA stays off the pipe. The package's own `.wasm` bytes are handed to its `init` as Emscripten's `wasmBinary`, because the package's default loader fetches by URL and fails under Node (measured 2026-09-16); which build, SIMD or not, is `wasm-feature-detect`'s answer, the detector the package calls. `sharp` stays a development dependency | — |
| **OCR recognition: a raster becomes characters and their boxes** (added 2026-09-10 ahead of Stage 6's rows) | **`tesseract.js-core`, driven directly, executing inside the engine host.** The matrix had a row for the OCR text **layer** — the embedding, `@cantoo/pdf-lib` — and none for the recognition, so D6's rows 2 to 7 all sat on a concern with no writer. `BUILD-PROMPT.md`:473 names `tesseract.js`; :806 puts only the TrOCR/onnxruntime stack in the download-on-demand class, so this one ships. **THE BINDING IS THE CORE AND NOT THE WRAPPER, 2026-09-10** ([ADR-0050](DECISIONS/0050-the-ocr-binding-is-tesseracts-core-driven-directly.md)): `tesseract.js@7.0.0` reaches `tr46@0.0.3`, which ships no licence file while declaring MIT, so `generateNotice.mjs` refuses it — correctly, and that refusal is not waivable by an ADR. `tesseract.js-core@7.0.0` is the package the WASM is in, declares **no dependencies**, ships an Apache-2.0 LICENSE, and is the version the wrapper itself pins. Measured against the same page the wrapper was probed with (`scripts/research/ocrCore.mjs`): **2,016 non-whitespace characters, 41 lines, 402 words all carrying a box, mean confidence 94** — identical, in 3.8–4.4 s against 4.8–5.2 s. The wrapper's worker orchestration is what this build does not want, since recognition already runs in a contained process; its model CDN is what ADR-0014 constraint 1 rules against; and `api.GetJSONText()` is the core's own renderer for the word tree. **[ADR-0014](DECISIONS/0014-ocr-stays-inside-the-engine.md)'s second ground is measured false for the engine this application loads** (`scripts/research/ocrSurface.mjs`, 2026-09-10): `tesseract`, `leptonica` and `ocr_` occur **0** times in the WASM binary the kernel's own `mupdf` import resolves to and **0** times across its four declaration and wrapper files, against **2, 4 and 9** in `monstera_mupdf.dll` — the artefact nothing under `packages/` loads. Each count carries its own anchor: libmupdf's error text is present in both binaries, and `PDFDocument`/`StructuredText` in the two surface files that declare the API, so a zero is an absence rather than a blind scan. **WHERE it runs is decided here rather than at a call site.** Recognition consumes a bitmap **we produced**, so the document-parse boundary invariant 25 governs was already crossed by the rasteriser — and that rasteriser is MuPDF's, in the host, by the row above. Running recognition beside it means no eight-megabyte bitmap crosses a pipe per page, no second rasteriser exists for OCR input (B3a), and containment is unchanged: tesseract.js parses no document bytes. Its language and data directory are **ours** and are never influenced by a document (ADR-0014 constraint 1). **AMENDED 2026-09-11 — THE ENGINE IS NAMED BY THE REQUEST** ([ADR-0052](DECISIONS/0052-a-second-recogniser-arrives-on-demand-and-reads-a-region.md)), because a second engine is a second answer to this row's own question and two writers for one concern is B3 unless the choice lives in exactly one place. `OcrRequest` carries the engine, the implementations sit behind one module boundary, and each answers a `RecognisedPage` — so nothing downstream chooses, and another engine adds a member rather than a surface. **THE LOCAL HANDWRITING ENGINE THIS WAS WRITTEN FOR IS REMOVED, 2026-09-18** ([ADR-0085](DECISIONS/0085-handwriting-is-read-by-a-service-and-the-local-engine-is-removed.md), the owner's decision): TrOCR's models are fine-tuned on the IAM Handwriting Database, whose terms are non-commercial research only, and the small model publishes no licence. So **Tesseract is the one local engine**, its runtime, models and ONNX Runtime files are gone, and handwriting is read by the network engines below, **on a region** as before. **AMENDED AGAIN 2026-09-12 — WHERE an engine runs is per engine, decided by what its input must reach** (ADR-0052's addition). *Both execute in the host* was true and about the two **local** engines; D6 row 8's Azure Document Intelligence is an HTTPS call, and invariant 25 gives the host no network — the same sentence that keeps every download in main. So `azure` executes in **`main`**, and what that gives up is precisely what the clause above protects: the raster crosses. Unavoidable and cheap in those terms, because the bytes are leaving the machine regardless and a pipe hop is nothing beside the upload. **No second rasteriser and no new channel**: `engine/snapshotRegion` already writes a region's PNG into the granted output directory, and it gains **the matrix mapping that PNG's pixels back to PDF user space** — six numbers — so the page transform, the crop origin and the `/Rotate` stay in the one process that holds them rather than being reconstructed main-side, which is FFFFFF-1 arriving in a third engine. Sending a rasterised region also keeps the service's answer in `pixel` rather than `inch`, so a third engine adds no fourth frame. **AMENDED 2026-09-13 — THE NETWORK ENGINES ARE A DECLARED SET, AND MAIN HOLDS ONE RECOGNISER PER ENGINE** ([ADR-0057](DECISIONS/0057-a-network-recogniser-is-keyed-by-engine-and-a-providers-key-is-the-providers.md)). A second network engine (Claude, D6) arrived where *runs in main* was three literals, one of them a ternary that would have recognised any new engine as handwriting. `NETWORK_OCR_ENGINES` in the contract is the only list; main's recognisers sit in a record keyed by it, so an engine without one does not compile; each takes the host's raster and frame. Claude's raster is sized so it is never resized server-side, its answer is schema-constrained and re-validated, and every stop reason but `end_turn` is refused by name. Its key is the provider's — one secret Stage 9's registry takes | — |
| In-place text editing (line/run rewriting), styled runs, HD render | **PDFium** | PDFium. **The GAP, recorded 2026-09-09 ([ADR-0049](DECISIONS/0049-the-editor-groups-its-own-engines-runs-and-a-person-confirms-the-grouping.md)): PDFium does not group runs into visual lines.** `FPDFText_CountRects` answers one rect per run — measured at 4 for four runs whether the two sharing a baseline are 170pt or 3pt apart (`scripts/research/pdfiumLines.mjs`) — so there is no engine answer for the substrate to own options over, which is the route [ADR-0034](DECISIONS/0034-the-text-substrate-owns-the-engines-options-not-its-own-clusterer.md) takes for the reading side. What PDFium **does** answer is `FPDFText_GetTextObject`, which maps a character to its page object (40 of 45 in that run; the other five are its own *generated* spaces, flagged by `FPDFText_IsGenerated`) — so a range of text converts to editable objects inside one engine's frame. The editor therefore owns a grouping, by vertical **overlap** and with no tunable constant, and a person confirms it before anything is written. **Since 2026-09-23 that grouping forms BLOCKS and a person edits them in place on the page** ([ADR-0096](DECISIONS/0096-text-is-edited-in-place-on-the-page-in-blocks-that-reflow.md)); a block that grows reflows into new objects made in the page's own fonts — `FPDFPageObj_CreateTextObj` round-trips 426 of 457 corpus runs, and a font that cannot carry what was typed is detected on the live text page (`scripts/research/pdfiumReflow.mjs`). **Since 2026-09-24 such a write falls back to a twin in the nearest standard font, and only a string no standard font carries refuses the edit** ([ADR-0097](DECISIONS/0097-a-page-is-translated-as-one-block-edit-and-a-font-that-cannot-carry-it-falls-back.md)): a subset's own font carries an accented Western string for 132 of 457 runs, the twin for 308 of 325 of the rest, `中` for 0 (`scripts/research/pdfiumFallbackFont.mjs`). One `editTextBlock` carries a page's blocks, which is how a translated page is one edit; a one-line block wraps at its column rather than at its own old end, and a translated block is FITTED — shrunk uniformly, never below 0.6, until it ends above its original bottom |
| Content composition: new document generation (markdown/CSV/TOC/image-to-PDF), drawing onto pages (watermark, headers/footers, Bates, OCR text layer) | **@cantoo/pdf-lib** — pdf-lib itself is unmaintained since 2021-11-06. **A new document composed from a file a person picks to import executes inside the compose host, never in `main`** ([ADR-0060](DECISIONS/0060-an-imported-source-is-parsed-in-a-contained-host-that-holds-no-document.md), 2026-09-13): the source is input someone else chose, and threat model §2 puts document parsing of any kind outside `main`. The Markdown parse is `markdown-it`'s, measured bounded on eight hostile inputs where `marked` aborted Node and `micromark` ran past 90 s | — |
...
| Text extraction, plain and layout-preserving | — (read-only) | **Plain: MuPDF** structured text, through the one substrate (`plainTextOf`), streamed to disk a page at a time so `main` holds at most the largest page ([ADR-0035](DECISIONS/0035-extracted-text-is-never-resident-in-main.md)). **Layout: Poppler's `pdftotext -layout`, 26.09.0, run as a separate CONTAINED process and never linked** (amended 2026-09-16, the owner's choice; [ADR-0071](DECISIONS/0071-layout-preserving-text-is-popplers-pdftotext-in-a-contained-process.md)). ENGINE-SPIKE H7's layout part (2026-09-14) found MuPDF's lines right on every fixture and **no layout output** under any option the engine names, and a row grid over those lines is the clustering ADR-0034 keeps out of the kernel — it failed a `/Rotate 90` page. So the layout is Poppler's own, through the external-converter seam: the contained launcher's `converter` kind, an AppContainer and a one-process job granted only the input's snapshot directory and the output directory, measured running there with both controls (an ungranted stage writes nothing; a path outside the pair is refused). The build is conda-forge's `poppler-26.09.0-h924501e_0`, provisioned as the twenty-two files `pdftotext.exe` loads, each package pinned by SHA-256, with every licence text and an offer of source in the notice; no component is GPL-2.0-only (FreeType is taken under the FTL). This is not the founding record's "when Poppler available", which stays withdrawn: that was a feature conditional on a binary being present, and this is a pinned writer **The COLUMNS half is executed** (2026-09-02, MuPDF 1.28.0): lines never merge across a gutter at 268pt or 60pt, and `FZ_STEXT_SEGMENT` yields column-major reading order — so no second engine and no clusterer of ours ([ADR-0034](DECISIONS/0034-the-text-substrate-owns-the-engines-options-not-its-own-clusterer.md)). **THE TABLES HALF IS EXECUTED, 2026-09-10** — it read *stay unexecuted* while no fixture contained a table, and the corpus now carries table-bearing documents. `FZ_STEXT_TABLE_HUNT` scored on against off over the eleven-document corpus: **two of the six documents carrying text change, at −1.5 and −17.5 points of line agreement against PDFium, and none improves**, with a constructed grid as the control that separates *found no table* from *the option never reached the engine*. It stays **off** for this row and stays a per-consumer opt-in — see §3.2, where that reading is corrected: it asked without `vectors`, which MuPDF's own table writer sets, and the *Finding a table* row takes the engine's table read as that writer asks it ([ADR-0073](DECISIONS/0073-a-table-is-the-engines-table-read-asked-as-its-own-table-writer-asks.md)), and note there why a fall in line agreement is not on its own the argument ([ADR-0013](DECISIONS/0013-pdfa-export-and-text-extraction-engines.md), corrected 2026-09-10) |
...
| Office document → PDF (import: `.docx`, `.xlsx`, `.pptx`) | **LibreOffice, headless, through the external-converter seam, never in `main`** ([ADR-0063](DECISIONS/0063-an-office-file-is-converted-by-a-pinned-libreoffice-in-a-contained-process.md), 2026-09-14). MuPDF, PDFium and `@cantoo/pdf-lib` read none of these formats, and threat model §2 keeps document parsing of any kind out of `main`. The PDF it writes opens by the one open route. **DEFERRED, 2026-09-22, the owner's decision** ([ADR-0092](DECISIONS/0092-office-import-is-deferred-because-libreoffices-pipe-cannot-be-contained.md)): contained, LibreOffice cannot start, because its single-instance check creates `\\.\pipe\OSL_PIPE_…`, an AppContainer may create pipes only in its own `LOCAL\` namespace, and the name is fixed in LibreOffice's code. **Nothing ships** — no command, no packaged LibreOffice, no NOTICE entry. The seam, its contract and `containedProgram` stay; weaker containment is rejected under threat model §2, and a patched LibreOffice is rejected as too costly for now | — |
| PDF/A-2b export (Stage 8) | **Ghostscript's `pdfwrite`, 10.08.0, through the external-converter seam, contained** — MuPDF has no PDF/A output mode and veraPDF validates without converting ([ADR-0013](DECISIONS/0013-pdfa-export-and-text-extraction-engines.md); built 2026-09-17, [ADR-0075](DECISIONS/0075-pdfa-2b-is-ghostscripts-pdfwrite-and-what-it-removes-is-reported.md)). Fixed arguments, compatibility policy 1, and the output intent passed inline from the sRGB profile in Ghostscript's own ROM — without it veraPDF fails the output on two rules, with it the generated H6 fixture and **11 of 11 corpus documents pass PDF/A-2b** (veraPDF 1.30.2, 2026-09-17, uncontained). **Ghostscript's exit code is not the report**: all three policies exited 0 on a document whose annotation it could not carry, and policy 2 printed *aborting conversion* while writing a passing file; so every line saying something is not permitted in PDF/A is shown to the person as what was removed. The build is conda-forge's `ghostscript-10.08.0-hac47afa_0`, five files, with each statically linked component's licence in the notice; none is GPL-2.0-only. veraPDF and its Java runtime validate in development only and never ship | — |
...
- **A grouping of our own is permitted where no engine answers, and only where a
  ...
  editor's reaches **the in-place editor and nothing else**
  ([ADR-0096](DECISIONS/0096-text-is-edited-in-place-on-the-page-in-blocks-that-reflow.md),
  which moved it off a dialog on 2026-09-23): each block is outlined around the
  very words it would replace, and nothing is written until a person types into it.

  So the rule stays checkable rather than becoming a judgement: **does this
  grouping's output reach any consumer other than the in-place editor a person
  answers, or a translation written back into the very blocks it was read
  from?** The second clause arrived 2026-09-24
  ([ADR-0097](DECISIONS/0097-a-page-is-translated-as-one-block-edit-and-a-font-that-cannot-carry-it-falls-back.md))
  and keeps what licensed the first: the output never becomes text taken as the
  document's content, and what the grouping decided is written where it was read,
  on the page, where undo reverses it. Any other caller is the moment it has
...
- **The text substrate owns the engine's OPTIONS and implements no clustering**
  ...
  **Correction, 2026-09-17: that reading asked the engine a question its own
  ...
  MuPDF's CSV writer sets `vectors` and `accurate-bboxes` beside `segment` and
  `table-hunt`, and `stext-table.c` proposes a table for every raft of vectors —
  so without `vectors` a ruling line proposes nothing, and on six generated grids
  every table came back two columns wide. The figures above stand as a measurement
  of `segment,table-hunt`; they are not a measurement of the engine's table
  finding, and they are not what the opt-in below rests on.

  **A third per-consumer read, `table`, from 2026-09-17** (ADR-0073): the shared
  set plus `vectors`, `accurate-bboxes` and `table-hunt`, the CSV writer's set
  taken rather than re-derived. Measured on MuPDF 1.28.0: all four ruled
  generated grids exact, both unruled ones still two columns wide, and on the
  corpus none of the lists, the two-column article or the résumé that
  `segment,table-hunt` alone read as tables. Excel export is its consumer; it is
  never part of the shared set, and its JSON goes through the same walk.

  **A second per-consumer opt-in, `structured`, from 2026-09-14**
  ([ADR-0065](DECISIONS/0065-a-tagged-documents-structure-is-the-engines-read-on-its-own-request.md)).
  It makes MuPDF follow a tagged document's structure tree, which is what the
  reading-order inspection reads and exactly why no shared consumer may take it:
  on 5 of the 8 corpus pages carrying text in tagged documents, tree order differs
  from stream order. The structure read asks for the shared set **plus** this one
  option, so it differs from the substrate's read by one named member, and its
  JSON is parsed by the same walk rather than a second reader.
...
- **A writer's `apply` takes ONE NAMED REQUEST, and every field is required**
  ([ADR-0069](DECISIONS/0069-a-writers-apply-takes-one-named-request.md),
  2026-09-16). `CommandExecution<W>.apply` is called by the bus and implemented
  six times — twice locally per engine, twice remotely, and delegated at the
  composition root — and it carried four positional parameters of which the last
  two were optional. **A function that ignores trailing parameters is assignable
  to one that passes them**, so every delegate, dispatch and remote half along
  that chain could drop `source` or `reads` and compile. One did, from
  2026-08-31 to 2026-09-16: `composition.ts`' live writer forwarded
  `(session, command)`, and **every command that copies from a second open
  document reached the engine host with no source** — Merge PDFs, Insert from
  PDF, Replace page and Import page as OCG layer, four rows that read *done* and
  had never worked in the running application.

  The seam now takes `ApplyRequest<W, K>` — `{ session, command, source, reads }`
  with all four **required**, `source` and `reads` typed `| undefined` rather
  than declared optional. Under `exactOptionalPropertyTypes` a caller that omits
  one does not compile, so the illegal state is unrepresentable rather than
  caught (B5): a forwarding delegate has one thing to forward, and dropping a
  field is an edit somebody has to write down.

  **What this does NOT close, stated because the guard is elsewhere:** `session`
  and `source` are the same type, so an object that names both and swaps them
  compiles exactly as the positional pair did. That is a wiring fact, not a
  shape fact, and the guard for it is
  `apps/desktop/src/compositionHost.test.ts`' case asserting which handle
  reached the host — the same kind of guard `pageMerge.test.ts` is for the
  `sources` axis's own one-way binding.
...
every other, and **the page cannot reach it**, by two mechanisms that answer two
different questions. The page's client is typed over renderer channels only, so
page code cannot name it at compile time; and **the bridge's `invoke` refuses a
preload channel's id**, because `invoke` is a raw transport that forwards any
string — without that refusal, page script could send `document.openDropped` a
path of its own spelling, and the type would stop nothing at runtime (amended
...
- **Chat history** ([ADR-0093](DECISIONS/0093-chat-history-is-off-by-default-encrypted-in-main-and-keyed-by-the-file.md)).
  **Off by default** (`ai.save-history`). When on, `main` keeps each file's
  assistant conversation in `chat-history.json`, encrypted with the keys'
  `SecretCipher`, keyed by a SHA-256 of the file's path so no path is stored in
  the clear; the renderer loads and saves by `DocId` and `main` refuses a save
  while the setting is off. Bounded to the contract's turn limits and 200
  files, least recently saved dropped first. *Clear chat history* in
...
- **Native code arrives two ways, and they have different rules.**
  ...
  **Downloaded as prebuilt binaries** (LibreOffice — a development provisioning
  only since 2026-09-22, never packaged, because Office import is deferred by
  [ADR-0092](DECISIONS/0092-office-import-is-deferred-because-libreoffices-pipe-cannot-be-contained.md) —
  `pdfium.dll`, and
  ...
  **An external converter is not an engine host** ([ADR-0063](DECISIONS/0063-an-office-file-is-converted-by-a-pinned-libreoffice-in-a-contained-process.md)).
  It is a program this build did not write, run once on one input, producing one
  output, and it holds no session and speaks no pipe protocol. What it shares with
  the hosts is invariant 25's containment, from the same AppContainer and job-object
  factory, generalised to launch a named executable with arguments, never copied.
  Its contract:
  - the executable comes from the provisioned tree, never `PATH` and never an
    installed copy;
  - no shell, and a picked file's name never reaches a command line;
  - one input and one output directory granted;
  - an isolated profile;
  - no macros;
  - time and memory bounds and kill-on-quit from the job object;
  - **its diagnostics are read whether it succeeds or not** (amended 2026-09-17,
    [ADR-0075](DECISIONS/0075-pdfa-2b-is-ghostscripts-pdfwrite-and-what-it-removes-is-reported.md)):
    Ghostscript exits 0 having removed what PDF/A cannot carry, and says so only in
    what it prints.

  **Where an artefact publishes a detached OpenPGP signature, provisioning verifies
  it** against a key pinned by its computed fingerprint, in one bootstrap-layer
  module the verifier commit writes. The SHA-256 pin says *these bytes*; the signature
  says *who built them*.
- **Network.** HTTPS only, host-locked per purpose, with an SSRF guard carrying
  ...
  **Cloud storage, 2026-09-22
  ([ADR-0091](DECISIONS/0091-cloud-storage-a-declared-provider-a-build-configured-client-and-a-working-copy.md)).**
  A cloud provider is a declared entry carrying the hosts its sign-in, its API and its
  downloads may use, and nothing reaches any other. Its OAuth client values are build
  configuration — the environment, or a packaged `oauth-clients.json` — and never source; a
  provider without them is *not configured in this build*. The sign-in is ADR-0059's loopback
  route, with Microsoft's redirect string `localhost` on a `127.0.0.1` listener. A cloud file is
  downloaded through the save pipeline into a working copy that opens as any document does,
  and *Save back* uploads it, refusing when the cloud file changed since it was opened.
...
- **External applications.** `main` hands exactly two things to the operating system's
  own handlers, and each is `entry.ts`' and refuses everything else: `openInBrowser`, an
  `https:` URL for a sign-in; and `openExternalEditor`, a path ending `.pdf` that
  `document.extract` has just written for *Edit page in external app*
  ([ADR-0062](DECISIONS/0062-a-page-edited-in-another-application-leaves-as-a-named-file-and-returns-by-the-one-open-route.md)).

  The edited file is **watched and never trusted**:

  - `main` watches its directory, filtered to its name — measured on Windows to survive
    every common save pattern;
  - an event only schedules a look, and an edit is a changed SHA-256 digest after a quiet
    second;
  - the renderer learns of it through `document.awaitExternalEdit`, a wait bounded at 30
    seconds on the one bridge function, never a push;
  - it comes back only when the person says so, through the one open route as a visible
    tab and the existing `replacePage` — refused, by the bus's version check on the
    `replacePage` it sends, if the document changed since the page left;
  - the file is the person's and is never deleted.
...
17. Memory is budgeted **per process**, each budget argued from what that
    ...
    **The baseline is itself budgeted**, and that third term is not bookkeeping:
    ...
    needs to create a contained engine host — `kernel32.dll`, `advapi32.dll` and
    `userenv.dll`, and nothing else. Its fixed cost should be within a small factor of a bare
...
27. **The renderer's Content-Security-Policy is exactly this list.** One
    ...
    **`style-src` grants `'self'` and three hashes — amended 2026-09-14
    ([ADR-0066](DECISIONS/0066-the-splitters-drag-cursor-is-admitted-by-hash.md)).**
    The hashes are the three drag-cursor styles `@zag-js/splitter`, ADR-0005's
    resizable-panel library, injects for a horizontal splitter; a hash admits that
    exact text and nothing else, measured with a control one space longer. What
    follows is why the grant began as `'self'` alone, and it still governs every
    other inline style.

    **It read `'self'` and nothing else until 2026-09-14, and this list carried
...
| `ground` | none; a colour the window's own background is at some point ([ADR-0106](DECISIONS/0106-a-translucent-surface-is-held-over-everything-it-can-sit-on.md)) |
| `glow` | none; a light laid over the ground, taken at its peak |
| `tint` | none; a surface's own gradient at its peak, over the surfaces it names `@on` |

**A translucent surface is held over everything it can sit on** (amended 2026-09-26,
[ADR-0106](DECISIONS/0106-a-translucent-surface-is-held-over-everything-it-can-sit-on.md)). It declares what it sits
`@over` — a surface, `ground`, or `any` (flat white and black, for anything that floats) — and every pair on it is held
to its floor against the worst colour it can present: the ground alone and under each glow, composited through each
surface beneath, with and without its tints. A translucent surface that names nothing beneath it is refused.
...
A stored overlay-on-page colour is declared in the `graphic` category with
`@on page`, which is what makes that 3:1 a pair the check evaluates; an accent
companion on the page stays derived.
...
- **Left section rail:** the eight feature sections — Home, Organize, Edit,
  ...
  — as labeled icons, and at its foot
...
- **Canvas** (the star, quiet chrome) → right contextual panel → **status bar**.
  **What the canvas shows follows the active section** (amended 2026-09-25,
  [ADR-0104](DECISIONS/0104-organize-shows-the-pages-as-a-grid-and-page-commands-act-on-the-selection.md)):
  Organize shows every page as a card in a grid — the thumbnail strip laid out
  across the canvas, with its reorder — where pages are selected, and page
  commands act on the selected pages (one reading, `targetPages`); every other
  section shows the reading view. Opening a page from the grid goes to it in
  the reading view, under Home.
...
| 2026-09-25 | **The bridge's `invoke` refuses a preload channel's id** (§5's preload-channel paragraph). ADR-0099 kept `document.openDropped` from the page because *"the page's client is typed over renderer channels only"*. That is a compile-time fact about page code; the bridge's `invoke` forwards any channel string at runtime, so page script could have called it with a path it spelt, which is the forgery the ADR exists to rule out. Found reading the ADR against the preload before building it; nothing was built on the gap. `invoke` now refuses every id in the contract's preload-channel list, and only the preload's own `openDropped` sends one. **Rejected:** checking the sender in `main`, which cannot tell the preload's `ipcRenderer` from the page's `invoke`, since both arrive as the same frame; a per-drop token, rejected in ADR-0099 for the same reason; a second, non-contract IPC name for the path, which is the same hole under a name nobody declared | §5's *"so page code cannot name it"*, added 2026-09-24 | [0099](DECISIONS/0099-a-dropped-file-is-opened-by-the-preload-and-its-path-never-reaches-the-page.md) (correction 2026-09-25) |
...
| 2026-09-19 | **Optimize is MuPDF's own image rewriter, reached natively, in the compose host, as a copy** (§3's optimize row). The owner's route: `pdf_rewrite_images`, which the WASM build does not bind, through the shim and koffi from a contained host. The compose host, because it holds no document and a native fault there poisons nothing; a copy, because the open document's writer is a live WASM session and the sizes must be compared before anything is kept — a result that is not smaller is not written. Measured over the corpus through two new shim exports: −10.6%, −11.6% and −25.6% at three settings, page counts held, 4 of 4 tagged documents keeping their tree, with a control that must shrink and one that must be seen tagged. The first product consumer of `monstera_mupdf.dll`; the document pipeline's migration stays unbuilt. Rejects Ghostscript (drops the structure tree from 4 of 4 tagged documents), a rewriter of our own (B3a), rewriting the open document, the MuPDF host, and a fourth host | §3's *"loaded by nothing the product runs"*, for the shim, from the feature commit | [0087](DECISIONS/0087-optimize-is-mupdfs-native-image-rewriter-in-the-compose-host.md) |
| 2026-09-19 | **A scanned table is read by a service that answers tables, and Tesseract is not a table engine** (§3's *Finding a table* row). The owner's decision: Azure through its Layout model, which returns table structure itself, and Claude asked for the table as structured output. ADR-0073 measured why the recognising engines were open — a scanned table's rulings are pixels, so recognised words reach MuPDF's table read as an unruled table — and left two routes, a service that answers tables or a grid over word boxes, which ADR-0034 refuses. Tesseract answers words and lines, so it could only take the second and is dropped. Both services run in `main` through their existing client (key, polling, Azure's delete), are sent the whole pages the person chose after the export dialog says so, and report spans, which become the sheet's merges; neither reports a cell's fill, which stays a stated limit. Rejects Tesseract with a grid, Tesseract's words through MuPDF's table read (ADR-0073's reading 3), a drawn region per table, and fills read from the raster | `BUILD-PROMPT.md`:502's *force-OCR* table-detection engine, and [ADR-0073](DECISIONS/0073-a-table-is-the-engines-table-read-asked-as-its-own-table-writer-asks.md) Decision 5 | [0086](DECISIONS/0086-a-scanned-table-is-read-by-a-service-that-answers-tables.md) |
| 2026-09-18 | **Handwriting is read by a service, and the local TrOCR engine is removed** (§3's recognition row). The owner's decision of 2026-09-17: TrOCR's models are fine-tuned on the IAM Handwriting Database, whose terms are non-commercial research only, and the small model publishes no licence — a restriction on use that no notice or pinned digest meets. The engine, its model downloader and cache, the ONNX Runtime files and their notice entries, its proof, its settings and Excel's local-handwriting option go; five reading engines become four. Handwriting means Azure Document Intelligence or Claude, already declared network engines. Azure's analysis result is deleted after every read, where it had stayed on the service for 24 hours; Claude's handwriting model is chosen by measuring Sonnet 5 against Opus 5; the handwriting dialog says to add a key in Settings; and the application removes the model cache it created. Rejects keeping the engine behind a notice (the terms restrict use, not attribution), a personal-use switch (nothing can verify it), and retraining on licensed data | `BUILD-PROMPT.md`:475's *local handwriting OCR (TrOCR …)* and :806's TrOCR/onnxruntime stack; ADR-0052's local engine and its 2026-09-17 runtime correction | [0085](DECISIONS/0085-handwriting-is-read-by-a-service-and-the-local-engine-is-removed.md) |
...
| 2026-09-17 | **PDF/A-2b is Ghostscript's `pdfwrite`, contained, and what it removes is reported** (§3, §8). The row read *unexecuted*. Measured: conda-forge's Ghostscript 10.08.0 (five files; statically linked components licence-checked, none GPL-2.0-only) passes veraPDF 1.30.2's PDF/A-2b on a generated H6 fixture and on 11 of 11 corpus documents once the output intent is passed inline from its ROM's sRGB profile, and fails two rules without it. Its exit code is 0 under every compatibility policy on a document whose annotation it cannot carry, and policy 2 prints *aborting* while writing a passing file — so policy 1, and its *not permitted in PDF/A* lines are the report. §8's converter contract gains: diagnostics are read on success too. veraPDF and a Java runtime validate in development and never ship | nothing; `BUILD-PROMPT.md`:505 is met | [0075](DECISIONS/0075-pdfa-2b-is-ghostscripts-pdfwrite-and-what-it-removes-is-reported.md) |
| 2026-09-17 | **A print is MuPDF's raster sent through the system print dialog and GDI, from `main`** (§3, §9.17). D10's *print* named the raster and forbade the DOM, and said nothing of how a raster reaches a printer; every Windows route goes through `gdi32.dll` and `comdlg32.dll`, which §9.17's *"nothing else"* excluded. Measured: two MuPDF rasters drawn with `StretchDIBits` into a *Microsoft Print to PDF* device context printed as two pages, each within a mean 0.14 of 255 of the raster sent for it and 17.5 from the other's. `main` binds the two libraries on the first print, not at startup. The same clause was short by `userenv.dll`, which `main` has bound since 2026-08-23 | nothing; `BUILD-PROMPT.md`:504-505 is met, not superseded | [0074](DECISIONS/0074-printing-is-mupdfs-raster-through-the-system-print-dialog-and-gdi.md) |
| 2026-09-17 | **A table is MuPDF's table read, asked as MuPDF's own CSV writer asks** (§3, §3.2). D10's *Excel: table detection* had no row, and the one reading of `FZ_STEXT_TABLE_HUNT` (2026-09-10) said it made extraction worse. That reading asked `segment,table-hunt`; `output-csv.c` also sets `vectors` and `accurate-bboxes`, and `stext-table.c` proposes a table per raft of vectors, so without them a ruling line proposes nothing. Measured on MuPDF 1.28.0 and 1.28.1: the option alone gave every one of six generated grids two columns; the CSV writer's set gave all four ruled grids exactly and both unruled ones two columns, and on the corpus it reported none of the lists and prose the option alone read as tables. A third per-consumer read, `table`; the 2026-09-10 figures are corrected in place in §3.2 and in ADR-0013. The four recognising engines stay undecided: a scanned table's rulings are pixels | ADR-0034's *"the first feature whose subject is a table owes the reading"* (a trigger, now fired); no founding clause | [0073](DECISIONS/0073-a-table-is-the-engines-table-read-asked-as-its-own-table-writer-asks.md) |
| 2026-09-17 | **Office Open XML exports are written by this build over `fflate`, in the engine host** (§3). D10's Word, PowerPoint and Excel rows had no writer. The record's answer, `exceljs`, cannot be adopted beside the record's own notice rule: measured in scratch trees, `exceljs` reaches five packages shipping no licence text, `docx` two and `pptxgenjs` two plus two high-severity `image-size` advisories — all three through `jszip` to `isarray@1.0.0`. `fflate` is one package, MIT, with its text. The parts each export needs are written here, composed a page at a time and streamed, so `main` holds a page's text and never the document's (ADR-0035; the ADR's same-day correction moved this from the engine host to `main`), and proven by opening the output in Word, PowerPoint or Excel. Rejected: the three libraries; an exception to the notice rule; vendoring a licence text a package does not ship; LibreOffice's PDF import; waiting for the owner | `BUILD-PROMPT.md`:67, *"Use exceljs"* — the *"never xlsx"* half stands | [0072](DECISIONS/0072-office-open-xml-exports-are-written-by-this-build-over-fflate.md) |
| 2026-09-16 | **Layout-preserving text is Poppler's `pdftotext`, run as a contained separate process** (§3). The *Text extraction* row's layout half read *NO WRITER, and choosing one is the owner's*; the owner chose Poppler through the external-converter seam. `pdftotext -layout` 26.09.0 runs under the contained launcher's `converter` kind, granted only the input and output directories — measured there with an ungranted-stage control and an outside-the-pair control. The build is conda-forge's, pinned per package; the twenty-two files it loads are licence-checked and none is GPL-2.0-only, FreeType being taken under the FTL where the package index's field read `GPL-2.0-only`. Rejected: a grid over MuPDF's lines amending ADR-0034; plain text only; linking Poppler into a host; the `poppler-windows` zip, which ships an unpinned environment and no licence texts | `BUILD-PROMPT.md` D10's "layout-preserving when Poppler available", already withdrawn as a conditional by ADR-0013 | [0071](DECISIONS/0071-layout-preserving-text-is-popplers-pdftotext-in-a-contained-process.md) |
| 2026-09-16 | **A page image is encoded as WebP by libwebp's WASM, in the engine host** (§3). D10's *Pages → PNG / JPEG / WebP* had a writer for two formats: MuPDF 1.28.0's pixmap and Electron's `nativeImage` encode no WebP. The owner chose `@jsquash/webp` on 2026-09-16. It encodes the pixmap MuPDF's export rasteriser already draws, inside the host beside it, with the package's own `.wasm` bytes handed to its `init` because the default loader fetches by URL and fails under Node. Rejected: promoting `sharp` (a second image stack, and a development dependency the owner kept out of production); encoding in `main` (the page's buffer across the pipe for nothing); a WebP writer of our own (a second opinion about libwebp's format); dropping WebP from the row | `BUILD-PROMPT.md`:500 names *Pages → PNG/JPEG/WebP* and no encoder | [0070](DECISIONS/0070-a-page-image-is-encoded-as-webp-by-libwebps-wasm-in-the-engine-host.md) |
| 2026-09-16 | **A writer's `apply` takes one named request, and every field is required** (§3.2). `CommandExecution<W>.apply` — ADR-0023 Decision 10's seam, the one member every command crosses to reach a session — carried four positional parameters with `source` and `reads` optional, because the interface is generic over the kinds routed to a writer rather than over one kind. **A function that ignores trailing parameters is assignable to one that passes them**, so each of the six implementations and four forwarding sites on that member could drop either and compile, and each short one was indistinguishable from a correct writer whose table declares neither. One was a drop: `composition.ts`' live MuPDF writer forwarded `(session, command)` from `9c53f05` (2026-08-31, written before a command could name a second document) to `631ff46` (2026-09-16), so from `238cf2f` (2026-09-05) **every command copying from a second open document reached the engine host with `source === undefined`** — Merge PDFs, Insert from PDF, Replace page and Import page as OCG layer, four rows reading *done* that had never worked in the running application. Both halves of the wired-tools pair were green because **the delegate is between them and neither crosses it**; the live run found it. Decided: `ApplyRequest<W, K>` = `{session, command, source, reads}`, all four **required**, the last two typed `\| undefined` rather than declared optional — under `exactOptionalPropertyTypes` an omitted key does not compile (B5), a forwarding delegate has one thing to forward, and `localPdfiumExecution`'s `_source?: never` placeholder stops being needed. **Does not close a `session`/`source` swap**, which is the same type and is a wiring fact: `compositionHost.test.ts` asserts which handle reached the host, as `pageMerge.test.ts` does for the `sources` axis's own one-way binding. Rejected: fixing the four rows alone (Rule 0's half-fix); a lint rule about delegate arity (a second opinion about assignability, B3a); required positional parameters (two adjacent bare `undefined`s at eleven call sites, the transposition risk restored); per-kind derivation (the correlated-union limit every writer file already casts around); branding the positions (the brand would be applied at the call site that is wrong) | Nothing in the founding record names this member's parameter list. `commandRouting.ts`' own note that the optionality *"makes this a one-line change rather than four"* is superseded: that convenience is the defect's mechanism | [ADR-0069](DECISIONS/0069-a-writers-apply-takes-one-named-request.md) |
| 2026-09-14 | **A tagged document's structure is MuPDF's, read on its own request** (§3, §3.2). D8's *reading-order / tagged-PDF inspection* had no §3 row, and MuPDF's type surface declares no structure-tree API, so the open-looking route was a `/StructTreeRoot` walker of our own — a second opinion about the standard, in the shape ADR-0034 rejected. **Measured on MuPDF 1.28.0**: the engine's `structured` option returns the tag roles, nested, in structure-tree order, independent of stream and visual order; the product options alone give segmentation's `Div` blocks in stream order; the product options plus `structured` match `structured` alone on the fixture. On the owner's corpus 4 of 11 documents are tagged and on 5 of their 8 text pages the orders differ. Decided: `structured` is a per-consumer opt-in named in the substrate module and never shared; the read asks for the product set plus it, on `engine/page-text` through a closed request field, parsed by the one reader; no sibling channel, since the largest tagged page read is 0.397% of the bound. Rejected: our own walker; `structured` for every consumer; `structured` without the product options; a sibling channel; waiting for an engine API | Nothing: `BUILD-PROMPT.md`:491 names the feature and no mechanism | [ADR-0065](DECISIONS/0065-a-tagged-documents-structure-is-the-engines-read-on-its-own-request.md) |
| 2026-09-14 | **Text extraction's layout half has no writer, and choosing one is the owner's** (§3). The row named MuPDF for "plain and layout-preserving". ENGINE-SPIKE H7's layout part, executed today against `pdftotext -layout` (Xpdf 4.00) on generated fixtures with the generator's own truth, found MuPDF's lines right on every fixture and **no layout output**: `asText()` gives one line per text line under every option the engine names, and an unknown option throws, so the options reach it. A row grid over those lines is the clustering ADR-0034 keeps out of the kernel, and it failed a `/Rotate 90` page when laid out in display space. So the row now says plain is MuPDF's — streamed a page at a time under ADR-0035 — and the layout writer is undecided among a grid amending ADR-0034, Poppler or Xpdf through the external-converter seam, and plain only | `BUILD-PROMPT.md`:500's *"layout-preserving when Poppler available"*, already withdrawn as a conditional by ADR-0013; this row's own claim that MuPDF covers layout | [ADR-0013](DECISIONS/0013-pdfa-export-and-text-extraction-engines.md), corrected 2026-09-14 |
| 2026-09-14 | **A page imported as an optional-content layer is MuPDF's, in the writer that already owns `/OCProperties`** (§3). D9's *Import page as OCG layer* straddled two rows with different writers: *drawing onto pages* is `@cantoo/pdf-lib`'s, and `/OCProperties` is written by `layers.ts` alone, after MuPDF's own layer API was measured not to survive save. Built on the first row it would have given `/OCProperties` a second writer. Decided: one §3 row for the operation, to MuPDF. The source page's content becomes a Form XObject carrying `/OC`, with inheritables pushed down and one graft map, and the group joins `/OCGs` and `/D/Order`. **Measured before it was written**: MuPDF 1.28.0 lists the layer after save and reopen, and with `/D /OFF` renders byte-identically to the untouched page; PDF.js 6.2.108, the renderer, draws the source's content at its place when on and hides it when off. Rejected: pdf-lib; marked-content wrapping (`BDC … EMC`, which needs a `/Properties` resource and a content edit); `setLayerVisible`; flattening ([ADR-0064](DECISIONS/0064-a-page-imported-as-a-layer-is-mupdfs-because-the-layer-is.md)) |
| 2026-09-14 | **An Office file is converted by a pinned LibreOffice through an external-converter seam that shares the hosts' containment and none of their protocol** (§3, §8). D9's *Office import* had no matrix row, and the seam ADR-0013 said LibreOffice "already requires" existed in no law and no code. The contained factory launches one program with no arguments. Decided: a §3 row; a converter contract in §8 (provisioned executable, no shell, one granted input and output, isolated profile, no macros, job-object bounds); and the factory generalised rather than copied. **Provisioning verifies TDF's detached signature as well as the SHA-256**, in one `node:crypto` module, because `gpg` is ambient. **The MSI is fetched from four pinned mirrors**, because both TDF hosts answer it with a 302 to a mirror chosen by location, which no host lock can follow. Rejected: converting in `main`; a fourth engine host; the machine's installed LibreOffice; an online service; the digest alone; following TDF's redirect with the lock loosened. Unmeasured, and read by the provisioning commit before any feature: administrative unpack without elevation, headless conversion, `soffice.exe` inside an AppContainer, forcing macros off, and the unpacked size ([ADR-0063](DECISIONS/0063-an-office-file-is-converted-by-a-pinned-libreoffice-in-a-contained-process.md)) |
| 2026-09-14 | **A page index in a command payload names the version it was read at** (the command table's `targets` axis, first stated by the 2026-09-06 row). ADR-0062 Decision 5 refuses a reimport when the document moved since the page left, and did not say where; a comparison before dispatch is a race across the lane. `replacePage` gains `version`, `CommandTargets` gains `'page'`, and the bus's `#refuseIfStale` makes the refusal atomic, as it does for the nine kinds that name an annotation, a field or a text object. `replacePage` is renderable, so the payload change is an amendment. | ADR-0062 Decision 5's unstated comparison; `replacePage`'s `targets: 'none'` | [0062](DECISIONS/0062-a-page-edited-in-another-application-leaves-as-a-named-file-and-returns-by-the-one-open-route.md), correction 2026-09-14 |
| 2026-09-13 | **A page edited in another application leaves as a named file and returns by the one open route** (§8's external-applications bullet; threat model §1.11 and §2's row for `main`). D9's *Edit page in external app & reimport* needed four things nothing registered: a file another program can reach, the operating system's handler for it, noticing its save, and putting the page back — on a bridge of one `invoke` function with no push. Decided: extract to a file the person names; `openExternalEditor` launches only a `.pdf` this application just wrote; a directory watch filtered to the name, whose events only schedule a digest check after a quiet second (measured on Windows under both runtimes: every save pattern reaches the watcher, and every event arrives twice); `document.awaitExternalEdit`, a wait bounded at 30 s; and a reimport the person asks for, through the one open route as a visible tab and `replacePage`, refused if the document's version moved. | Nothing: the founding record's catalog line names the row and nothing more, and no sentence in this document said `main` hands the operating system nothing else. `entry.ts`' comment calling `openInBrowser` *the one route by which this application opens a URL outside itself* stays true — the new launcher opens a file, not a URL | [0062](DECISIONS/0062-a-page-edited-in-another-application-leaves-as-a-named-file-and-returns-by-the-one-open-route.md) |
