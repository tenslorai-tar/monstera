...
**seven engine-agnostic** — `probe-containment`, `open`, `serialise`, `close`,
`apply`, `capture`, `invert` — and **twelve MuPDF document-model reads**:
...
`engine/open` parses once and discards it**, so `open-failed` means *this
engine cannot read this document* from both hosts at the same point in the same
protocol.
