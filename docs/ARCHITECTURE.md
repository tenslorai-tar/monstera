...
**A mutation reaches the screen through the view model, not through the bytes**,
and that is a property of this design rather than an accident of it. A
`DocumentRecord`'s bytes are `readonly` and a command never replaces them: the
mutation lands in the engine session, so `document.readRange` serves the
pre-command document for the whole life of an open file (measured 2026-08-30).
§3.2's *"PDF.js is never a source of truth. It renders"* is what makes that
correct — the parser is handed the kernel's rotation and overruled on the one
value stale bytes cannot carry.
