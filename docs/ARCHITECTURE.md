...
**AND THAT SERIALISE CARRIES THE COMMAND'S PURPOSE.**
`EngineWriter.serialise(session, purpose)` takes `'ordinary' | 'removal'`, which
the command declares and the adapter maps to its engine's write options —
MuPDF's `garbage` for a removal, an empty option string otherwise. There is no
default and no setting, which is §4's rule kept rather than restated, and the
mapping is exhaustive over the union so an adapter that handles neither case is
a compile error rather than a document that quietly keeps what it removed
([ADR-0045](DECISIONS/0045-a-removals-garbage-collection-belongs-to-the-command.md)).
The purpose crosses to the engine host as a bounded enum on `engine/serialise`,
because that is where the session lives.
...
A command declares **`asset: 'none' | 'image'`** on the seam, beside `sources`
and `reads`. Main writes the bytes into the session's `snapshotDirectory` — the
